...
const pool = require('../db'); // Menggunakan pool yang sudah ada dari db.js
require('dotenv').config({ path: require('path').resolve(__dirname, '../.env') }); // Path yang lebih robust

/**
 * Skrip untuk membuat pengguna admin baru dari baris perintah.
 *
 * Cara Penggunaan:
 * node backend/create-admin.js "<nama_lengkap>" <email> <password>
 *
 * Contoh:
 * node backend/create-admin.js "Admin Utama" "admin@kopkaka.com" "password123"
 */
const createAdmin = async () => {
    const args = process.argv.slice(2);
    if (args.length < 3) {
        console.error('Penggunaan: node backend/create-admin.js "<nama_lengkap>" <email> <password>');
        process.exit(1);
    }
